...
Deno.serve(async (req) => {
  try {
    ...
    for (const { match_type, key, records } of groups) {
      // When a forced survivor is specified, only process the group containing it
      ...
      const survivor = forced_survivor_id
        ...
        : pickSurvivor(active, eventCountsById);
    ...
    if (!dry_run && report.duplicates_marked_inactive.length > 0) {
      ...
      for (const dup of report.duplicates_marked_inactive) {
        await base44.asServiceRole.entities.AuditLog.create({
          ...
          after_data: { status: 'Inactive', canonical_key: `series:DUPLICATE_OF:${dup.survivor_id}`, survivor_id: dup.survivor_id, survivor_name: dup.survivor_name },
          performed_by: user.id,
          performed_by_name: user.full_name,
          timestamp: now,
          notes: `Merged into survivor ${dup.survivor_id} (${dup.survivor_name}) via repairDuplicateSeriesRecords`,
        }).catch(() => {});
      }
    }
