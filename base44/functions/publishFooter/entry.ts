...
import { validateFooterConfig } from '../../shared/footerDefaults.ts';
 ...
 * Records a FOOTER_CONFIG_PUBLISHED audit event via the existing AuditLog entity.
 */
export default async function (req) {
  try {
    ...
    const validation = validateFooterConfig(record.draft);
    if (!validation.valid) {
    ...
    await db.FooterSettings.update(record.id, {
    ...
    // Audit log — best-effort, never blocks publish
    try {
      await db.AuditLog.create({
        entity_type: 'FooterSettings',
        entity_id: record.id,
        entity_name: 'Footer Configuration',
        action: 'lifecycle_change',
        performed_by: user.id,
        performed_by_name: user.full_name || user.email || user.id,
        timestamp: now,
        notes: 'FOOTER_CONFIG_PUBLISHED',
      });
    } catch {
