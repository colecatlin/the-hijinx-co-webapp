...
Deno.serve(async (req) => {
  try {
    ...
    // Authorization: admin, or event/series/track collaborator
    if (user.role !== 'admin') {
      ...
      if (!allowed) {
        return Response.json({ error: 'Forbidden: must be admin or event/series/track collaborator' }, { status: 403 });
      }
    }
