...
import { validateSeoConfig } from '../../shared/seoDefaults.ts';
 ...
 * Records a SEO_CONFIG_PUBLISHED audit event via the existing AuditLog entity.
 */
export default async function (req) {
  try {
    ...
    const validation = validateSeoConfig(record.draft);
    if (!validation.valid) {
    ...
    await db.SeoSettings.update(record.id, {
    ...
    // Audit log — best-effort, never block publish
    try {
      await db.AuditLog.create({
        entity_type: 'SeoSettings',
        entity_id: record.id,
        entity_name: 'SEO Configuration',
        action: 'lifecycle_change',
        performed_by: user.id,
        performed_by_name: user.full_name || user.email || user.id,
        timestamp: now,
        notes: 'SEO_CONFIG_PUBLISHED',
      });
    } catch {
