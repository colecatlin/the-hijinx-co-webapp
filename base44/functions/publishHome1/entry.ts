...
import { validateHome1Config } from '../../shared/home1Defaults.ts';
 ...
 * Records a HOME_CONFIG_PUBLISHED audit event via the existing AuditLog
...
export default async function (req) {
  try {
    ...
    const validation = validateHome1Config(record.draft);
    if (!validation.valid) {
    ...
    try {
      await db.AuditLog.create({
        entity_type: 'Home1Settings',
        entity_id: record.id,
        entity_name: 'Home Configuration',
        action: 'lifecycle_change',
        performed_by: user.id,
        performed_by_name: user.full_name || user.email || user.id,
        timestamp: now,
        notes: 'HOME_CONFIG_PUBLISHED',
      });
    } catch {
