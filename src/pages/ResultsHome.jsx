...
export default function ResultsHome() {
  ...
  // Build a map of event_id -> top 3 finishers
  const podiumByEvent = useMemo(() => {
    ...
    for (const event of events) {
      ...
      // fallback: no session_type filter if no Final results found
      const finalResults = eventResults.length > 0
        ? eventResults
        : allResults
            .filter(r => r.event_id === event.id && r.position)
            .sort((a, b) => a.position - b.position);

      map[event.id] = finalResults.slice(0, 3).map(r => {
              ...
              return (
                <div key={event.id} className="flex items-stretch border border-gray-100 hover:border-gray-300 transition-colors group">
                  ...
                  <div className="flex-1 px-5 py-4">
                    <h3 className="font-bold text-sm text-[#0A0A0A] mb-2 group-hover:underline">{event.name}</h3>
                    {podium.length > 0 ? (
