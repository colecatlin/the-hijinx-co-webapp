...
export default function ManageAccess() {
  ...
  // Filter invitations by type and role
  const filteredInvitations = useMemo(() => {
    let filtered = allInvitations;
    if (filterEntityType !== 'all') {
      filtered = filtered.filter(inv => inv.entity_type === filterEntityType);
    }
    if (filterRole !== 'all') {
      filtered = filtered.filter(inv => inv.role === filterRole);
    }
    return filtered;
              ...
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Email</th>
                      <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Entity Type</th>
                      <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Entity Name</th>
