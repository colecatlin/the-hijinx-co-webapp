...
export default function ManageEntityClaims() {
  ...
  const handleAction = async (claimId, action) => {
    ...
    const res = await base44.functions.invoke('approveEntityClaim', { claim_id: claimId, action }).catch(() => null);
  ...
  return (
    <ManagementLayout currentPage="ManageEntityClaims">
      <ManagementShell
        title="Entity Claims"
        subtitle="Review ownership claim requests submitted by users."
        maxWidth="max-w-4xl"
      >
        {/* Filter tabs */}
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {claims.map(claim => {
              ...
              return (
                <div key={claim.id} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-xs text-gray-400">{claim.entity_type}</span>
                      </div>
                      <p className="text-xs text-gray-500 mt-1">
                        By <strong>{claim.user_email}</strong>
                        {claim.created_date && ` · ${format(new Date(claim.created_date), 'MMM d, yyyy')}`}
                      </p>
                      {claim.justification && (
                        <p className="text-xs text-gray-600 mt-1.5 italic bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                          "{claim.justification}"
                        </p>
                      )}
                  ...
                  {isPending && (
                    <div className="flex gap-2 flex-shrink-0 sm:mt-0 mt-1">
                      <Button
                        size="sm"
                        variant="outline"
                      ...
                      >
                        {actionLoading === claim.id + 'reject' ? <Loader2 className="w-3 h-3 animate-spin" /> : <XCircle className="w-3.5 h-3.5" />}
                        Reject
                      </Button>
                      <Button
                        size="sm"
                      ...
                      >
                        {actionLoading === claim.id + 'approve' ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                        Approve
                      </Button>
                    </div>
                  )}
