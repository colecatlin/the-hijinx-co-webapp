...
const STATUS_COLORS = {
  ...
  Live:            'bg-motion/10 text-motion',
  Completed:       'bg-motion/10 text-motion',
  Cancelled:       'bg-danger/10 text-danger',
};

function SectionLabel({ label }) {
  return (
    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-foreground-quiet mb-3">
      {label}
    </p>
  );
}
...
function ReadOnlyField({ label, value, mono = false }) {
  return (
    <div className="space-y-1">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet">{label}</p>
      <p className={`text-sm text-foreground-secondary ${mono ? 'font-mono' : ''}`}>
        {value || <span className="text-foreground-quiet italic">—</span>}
      </p>
    </div>
  );
}
  ...
  if (!selectedEvent) {
    return (
      <div className="flex items-center justify-center h-32">
        <p className="text-xs text-foreground-quiet">No event loaded.</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-5 max-w-2xl">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-foreground-quiet">Event Settings</p>
            <p className="text-[11px] text-foreground-quiet mt-0.5">Core event information.</p>
          </div>
          <a
            href={fullMgmtUrl}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border border-divider text-foreground-quiet hover:text-foreground hover:border-divider transition-colors shrink-0"
          >
        ...
        {/* Read-only display */}
        <div className="p-4 bg-surface-elevated border border-divider/60 rounded-xl space-y-4">
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-foreground-quiet">Event Settings</p>
          <p className="text-[11px] text-foreground-quiet mt-0.5">
            Edit core event information. Track, series, and approval workflows are managed separately.
          </p>
        </div>
        <a
          href={fullMgmtUrl}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border border-divider text-foreground-quiet hover:text-motion hover:border-motion/40 transition-colors shrink-0"
        >
          <ExternalLink className="w-3 h-3" /> Full Event Management
        </a>
      </div>
      ...
      <div className="p-4 bg-surface-elevated border border-divider/60 rounded-xl space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            className={`bg-surface-interactive border-divider text-foreground text-sm h-9 ${errors.name ? 'border-danger' : ''}`}
          />
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label className="text-xs text-foreground-quiet uppercase tracking-wider">Season</Label>
            <Input
              value={formData.season}
              onChange={e => handleChange('season', e.target.value)}
              placeholder="e.g. 2025"
              className="bg-surface-interactive border-divider text-foreground text-sm h-9"
            />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-foreground-quiet uppercase tracking-wider">Round #</Label>
            <Input
              type="number"
        ...
        <div className="space-y-1.5">
          <Label className="text-xs text-foreground-quiet uppercase tracking-wider">Location Note</Label>
          <Textarea
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className={`bg-surface-interactive border-divider text-foreground text-sm h-9 ${errors.event_date ? 'border-danger' : ''}`}
            />
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className={`bg-surface-interactive border-divider text-foreground text-sm h-9 ${errors.end_date ? 'border-danger' : ''}`}
            />
      ...
      <div className="p-4 bg-surface-elevated border border-divider/60 rounded-xl space-y-4">
        ...
        <div className="space-y-1.5">
          <Label className="text-xs text-foreground-quiet uppercase tracking-wider flex items-center gap-1.5">
          ...
          <Input
            ...
            placeholder="https://..."
            className="bg-surface-interactive border-divider text-foreground text-sm h-9 font-mono"
          />
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs text-foreground-quiet uppercase tracking-wider flex items-center gap-1.5">
        ...
        {(formData.event_logo_url || formData.event_cover_image_url) && (
          <div className="flex gap-3 pt-1">
            {formData.event_logo_url && (
              <div className="w-12 h-12 rounded-lg border border-divider overflow-hidden bg-surface flex-shrink-0">
                <img src={formData.event_logo_url} alt="Logo" className="w-full h-full object-contain" />
              </div>
            )}
            {formData.event_cover_image_url && (
              <div className="flex-1 h-12 rounded-lg border border-divider overflow-hidden bg-surface">
      ...
      <div className="p-4 bg-surface-elevated border border-divider/60 rounded-xl space-y-4">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3">
          {/* Status */}
          <div className="space-y-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet">Status</p>
            <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold ${STATUS_COLORS[selectedEvent.status] || 'bg-surface-interactive text-foreground-secondary'}`}>
              {selectedEvent.status || '—'}
            </span>
          </div>
          {/* Track */}
          <div className="space-y-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet flex items-center gap-1">
              <MapPin className="w-3 h-3" /> Track
            </p>
            <p className="text-xs text-foreground-secondary">{selectedTrack?.name || <span className="text-foreground-quiet italic">—</span>}</p>
          </div>
          {/* Series */}
          <div className="space-y-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet flex items-center gap-1">
            ...
            <p className="text-xs text-foreground-secondary">{selectedSeries?.name || <span className="text-foreground-quiet italic">None</span>}</p>
          </div>
          {/* Event ID */}
          <div className="space-y-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet">Event ID</p>
            <p className="text-[10px] font-mono text-foreground-quiet truncate">{selectedEvent.id}</p>
          </div>
          {/* Track acceptance */}
          {selectedEvent.track_acceptance_status && (
            <div className="space-y-1">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet">Track Acceptance</p>
              <p className="text-xs text-foreground-quiet">{selectedEvent.track_acceptance_status}</p>
            </div>
          )}
          {/* Series acceptance */}
          {selectedEvent.series_acceptance_status && (
            <div className="space-y-1">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-quiet">Series Acceptance</p>
              <p className="text-xs text-foreground-quiet">{selectedEvent.series_acceptance_status}</p>
            </div>
          )}
        </div>

        {/* Approval workflow notice */}
        <div className="flex items-start gap-2 pt-2 border-t border-divider/60">
          <Info className="w-3.5 h-3.5 text-foreground-quiet shrink-0 mt-0.5" />
          <p className="text-[10px] text-foreground-quiet leading-relaxed">
      ...
      <div className="p-4 bg-surface-elevated border border-divider/60 rounded-xl">
      ...
      <div className="flex items-center gap-3 pt-1">
        <Button
          ...
          className="gap-2 bg-motion hover:bg-motion-hover disabled:opacity-40 text-white text-sm h-9"
        >
          {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
          {saving ? 'Saving…' : 'Save Changes'}
        </Button>
        {dirty && !saving && (
          <span className="text-[10px] text-warning font-mono">Unsaved changes</span>
        )}
      </div>
    </div>
  );
