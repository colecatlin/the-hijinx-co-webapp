...
export default function TrackFlipCard({ track, disciplines = [], media }) {
  ...
  const handleProfileClick = (e) => {
    e.stopPropagation();
    navigate(createPageUrl('TrackProfile', { slug: track.slug }));
  };
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {track.track_type && (
              <div>
                ...
                <div className="text-sm font-bold text-[#232323]">{track.track_type}</div>
              </div>
            )}
            {track.founded_year && (
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Founded</div>
                <div className="text-sm font-bold text-[#232323]">{track.founded_year}</div>
              </div>
            )}
            {track.length_miles && (
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Length</div>
                <div className="text-sm font-bold text-[#232323]">{track.length_miles} mi</div>
              </div>
            )}
            {track.turns_count && (
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Turns</div>
                <div className="text-sm font-bold text-[#232323]">{track.turns_count}</div>
              </div>
            )}
            {disciplines.length > 0 && (
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Disciplines</div>
                <div className="text-sm font-bold text-[#232323]">{disciplines.length}</div>
              </div>
            )}
            {track.status && (
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Status</div>
                <div className="text-sm font-bold text-[#232323]">{track.status}</div>
              </div>
            )}
          </div>
