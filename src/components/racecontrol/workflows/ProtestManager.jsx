...
const STATUS_COLOR = {
  ...
  Closed: 'bg-gray-700 text-foreground-quiet',
  Rejected: 'bg-gray-700 text-foreground-quiet',
};
      ...
      <DialogContent className="bg-gray-950 border-divider text-white max-w-md max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="space-y-1.5">
            <Label className="text-foreground-quiet text-xs">Protest Type *</Label>
          ...
          <div className="space-y-1.5">
            <Label className="text-foreground-quiet text-xs">Filing Driver ID *</Label>
            <Input value={form.filing_driver_id} onChange={e => set('filing_driver_id', e.target.value)}
              className="bg-surface-elevated border-divider text-white" placeholder="Driver ID…" required />
          </div>

          <div className="space-y-1.5">
            <Label className="text-foreground-quiet text-xs">Against Driver ID (optional)</Label>
          ...
          {sessions.length > 0 && (
            <div className="space-y-1.5">
              <Label className="text-foreground-quiet text-xs">Session (activates hold on select)</Label>
          ...
          <div className="space-y-1.5">
            ...
            <Input value={form.rule_reference} onChange={e => set('rule_reference', e.target.value)}
              className="bg-surface-elevated border-divider text-white" placeholder="e.g. 8.2.1" />
          </div>

          <div className="space-y-1.5">
            <Label className="text-foreground-quiet text-xs">Description *</Label>
            <Textarea required value={form.description} onChange={e => set('description', e.target.value)}
              className="bg-surface-elevated border-divider text-white h-20 resize-none" placeholder="Grounds for protest…" />
          </div>
...
function ProtestDetailDrawer({ protest, open, onClose, eventId, sessions, onRefresh }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-gray-950 border-divider text-white max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-white flex items-center gap-2">
        ...
        <div className="space-y-5 mt-1">
          ...
          <div>
            ...
            <p className="text-sm text-foreground-secondary leading-relaxed">{protest.description}</p>
          </div>

          {protest.rule_reference && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-600 mb-1">Rule Reference</p>
              <p className="text-xs text-foreground-quiet">{protest.rule_reference}</p>
            </div>
          )}
          ...
          {protest.steward_notes && (
            <div>
              ...
              <p className="text-xs text-foreground-quiet whitespace-pre-wrap">{protest.steward_notes}</p>
            </div>
          )}
          ...
          {canManage && isActive && (
            <>
              {/* Assign steward */}
              <div className="p-3 bg-surface-elevated/60 border border-divider rounded-lg space-y-2">
                ...
                <div className="flex gap-2">
                  <Input value={assigneeId} onChange={e => setAssigneeId(e.target.value)}
                    className="bg-surface-elevated border-divider text-white text-xs h-8 flex-1" placeholder="User ID…" />
              ...
              <div className="p-3 bg-surface-elevated/60 border border-divider rounded-lg space-y-2">
                ...
                <Textarea value={stewardNotes} onChange={e => setStewardNotes(e.target.value)}
                  className="bg-surface-elevated border-divider text-white text-xs h-16 resize-none" placeholder="Notes…" />
              ...
              {['Accepted', 'Under Review', 'Hearing Scheduled'].includes(protest.status) && (
                <div className="p-3 bg-surface-elevated/60 border border-divider rounded-lg space-y-2">
                  ...
                  <Textarea value={decisionText} onChange={e => setDecisionText(e.target.value)}
                    className="bg-surface-elevated border-divider text-white text-xs h-20 resize-none"
    ...
    <div className="space-y-2">
      ...
      {displayed.map(pro => (
        <button key={pro.id} onClick={() => setSelectedProtest(pro)}
          className="w-full text-left rounded-lg border border-divider bg-surface-elevated/40 hover:border-divider hover:bg-surface-elevated/70 p-3 transition-colors">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs text-foreground-quiet truncate">{pro.protest_type}</span>
            </div>
