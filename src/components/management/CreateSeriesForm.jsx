  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold">{createdSeries ? createdSeries.name : 'New Series'}</h2>
          {createdSeries && <p className="text-sm text-gray-600">Complete series setup</p>}
        </div>
      </div>
      ...
      <Tabs defaultValue="core" className="w-full">
        <TabsList className="w-full flex flex-wrap h-auto gap-1 bg-gray-100 p-1 rounded-lg">
          ...
          <TabsTrigger value="format" className="text-xs" disabled={!createdSeries}>Format</TabsTrigger>
          <TabsTrigger value="classes" className="text-xs" disabled={!createdSeries}>Classes</TabsTrigger>
          <TabsTrigger value="calendar" className="text-xs" disabled={!createdSeries}>Calendar</TabsTrigger>
          <TabsTrigger value="media" className="text-xs" disabled={!createdSeries}>Media</TabsTrigger>
          <TabsTrigger value="governance" className="text-xs" disabled={!createdSeries}>Governance</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="format" className="mt-6">
          {createdSeries && <SeriesFormatSection seriesId={createdSeries.id} />}
        </TabsContent>

        <TabsContent value="classes" className="mt-6">
          {createdSeries && <SeriesClassesSection seriesId={createdSeries.id} />}
        </TabsContent>

        <TabsContent value="calendar" className="mt-6">
          {createdSeries && <SeriesEventsSection seriesId={createdSeries.id} />}
        </TabsContent>

        <TabsContent value="media" className="mt-6">
          {createdSeries && <SeriesMediaSection seriesId={createdSeries.id} />}
        </TabsContent>

        <TabsContent value="governance" className="mt-6">
          {createdSeries && <SeriesGovernanceSection seriesId={createdSeries.id} />}
        </TabsContent>
      </Tabs>
    </div>
  );
