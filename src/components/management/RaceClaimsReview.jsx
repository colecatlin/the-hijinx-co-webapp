...
import { CheckCircle2, XCircle, ExternalLink, AlertCircle, Clock, User, Loader2 } from 'lucide-react';
import { format } from 'date-fns';
import { toast } from 'sonner';

/**
 * RaceClaimsReview — shared race-result (DriverClaim) review panel.
 * Reused by both the standalone ManageDriverClaims page and the consolidated
 * "Race Result Claims" segment inside Manage Access, so all approval work
 * lives in a single source of truth.
 */
export default function RaceClaimsReview() {
  ...
  const updateClaimMutation = useMutation({
    mutationFn: async ({ claimId, status, notes }) => {
      const res = await base44.functions.invoke('verifyDriverClaim', {
        claim_id: claimId,
        action: status,
    ...
    onSuccess: async (_, variables) => {
      ...
      if (driver?.contact_email) {
        const statusText = variables.status === 'verified' ? 'verified' :
          variables.status === 'rejected' ? 'rejected' : 'marked as duplicate';
        try {
    ...
    onError: (error) => toast.error('Failed to update claim: ' + error.message),
  });
      ...
      <Card className="p-4 hover:shadow-md transition-shadow">
        ...
        {claim.status === 'pending' && (
          <div className="flex gap-2 mt-4">
            <Button
              size="sm"
              onClick={() => { setSelectedClaim(claim); setReviewAction('verified'); }}
            >
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => { setSelectedClaim(claim); setReviewAction('rejected'); }}
            >
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => { setSelectedClaim(claim); setReviewAction('duplicate'); }}
            >
  ...
  return (
    <div className="space-y-6">
      {/* Local status summary (segmented in Manage Access) */}
      <div className="grid grid-cols-3 gap-3">
        <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <Clock className="w-4 h-4 text-amber-600" />
          <span className="text-sm text-amber-800 font-medium">Pending: <strong>{pendingClaims.length}</strong></span>
        </div>
        <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
          <CheckCircle2 className="w-4 h-4 text-green-600" />
          <span className="text-sm text-green-800 font-medium">Verified: <strong>{verifiedClaims.length}</strong></span>
        </div>
        <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
          <XCircle className="w-4 h-4 text-red-600" />
          <span className="text-sm text-red-800 font-medium">Rejected: <strong>{rejectedClaims.length}</strong></span>
        </div>
      </div>
      ...
      {claims.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <p className="text-gray-500">No race result claims.</p>
        </Card>
      ) : (
        <div className="space-y-4">
          {pendingClaims.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-1">Pending ({pendingClaims.length})</p>
              {pendingClaims.map(claim => <ClaimCard key={claim.id} claim={claim} />)}
            </div>
          )}
          {verifiedClaims.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-1">Verified ({verifiedClaims.length})</p>
              {verifiedClaims.map(claim => <ClaimCard key={claim.id} claim={claim} />)}
            </div>
          )}
          {rejectedClaims.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-1">Rejected ({rejectedClaims.length})</p>
