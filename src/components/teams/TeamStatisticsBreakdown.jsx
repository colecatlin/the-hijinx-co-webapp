...
import { Trophy, Flag, Award, TrendingUp, Users, Truck } from 'lucide-react';

export default function TeamStatisticsBreakdown({ statistics }) {
  ...
  if (!statistics || !statistics.career) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
        ...
        <p className="text-gray-500 text-sm">No statistics available yet. Statistics will appear as the team competes.</p>
      </div>
    );
  }
  ...
  const careerCards = [
    ...
    { label: 'Vehicles', value: career.vehicles_count, icon: Truck },
  ];
  ...
  const breakdowns = [
    ...
    { id: 'manufacturer', label: 'By Manufacturer', data: statistics.by_manufacturer },
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {activeBreakdownData.data.map((item, idx) => {
                  const name = item.series_name || item.track_name || item.driver_name || item.manufacturer || item.class_name || item.season || 'Unknown';
                  return (
