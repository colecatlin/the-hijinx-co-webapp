...
export default function HomepageTicker() {
  ...
  return (
    <div className="relative bg-[#080808] border-y border-[#00FFDA]/15 overflow-hidden py-3 select-none">
      {/* Edge fades */}
      ...
      <div className="absolute left-6 top-1/2 -translate-y-1/2 z-20 hidden sm:flex items-center gap-2">
        ...
        <span className="font-mono text-[9px] tracking-[0.3em] text-[#00FFDA]/60 uppercase">Live</span>
      </div>
      ...
      >
        {repeated.map((item, i) => (
          ...
          >
            <span className="w-1 h-1 rounded-full bg-[#00FFDA]/40 flex-shrink-0 group-hover:bg-[#00FFDA] transition-colors" />
            <span className="font-mono text-[10px] tracking-[0.3em] text-white/35 group-hover:text-[#00FFDA] transition-colors whitespace-nowrap uppercase">
              {item.label}
            </span>
          </Link>
        ))}
