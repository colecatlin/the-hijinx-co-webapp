...
export default function ApparelShowcase({ products = [] }) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-stretch">
          ...
          >
            <ShopLink
              ...
              className="group relative flex flex-col justify-end min-h-[440px] overflow-hidden block"
              style={{ border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12 }}
            >
              <img
                src={featuredProduct?.cover_image_url || APPAREL_BG}
                alt="HIJINX Apparel"
                className="absolute inset-0 w-full h-full object-cover opacity-65 group-hover:opacity-82 group-hover:scale-[1.03] transition-all duration-700"
                style={{ filter: 'contrast(1.1) saturate(0.85)' }}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent" />
              <div className="absolute top-0 left-0 right-0 h-[2px]"
                style={{ background: 'linear-gradient(90deg, #E5FF0090 0%, transparent 60%)' }} />
              <div className="absolute top-0 left-0 bottom-0 w-[2px]"
                style={{ background: 'linear-gradient(180deg, #E5FF0055 0%, transparent 55%)' }} />

              <div className="relative p-8 md:p-10">
                <p className="font-mono text-[9px] tracking-[0.45em] text-[#E5FF00] uppercase font-bold mb-3">
                  HIJINX CO.
                </p>
                <h3 className="text-4xl md:text-6xl font-black text-white tracking-tight leading-none mb-2">
                ...
                {featuredProduct?.short_description && (
                  <p className="text-white/40 text-sm mt-3 mb-5 max-w-sm leading-relaxed">
                    {featuredProduct.short_description}
                  </p>
                )}
                <div
                  className="inline-flex items-center gap-2.5 px-5 py-3 text-xs font-black tracking-wider uppercase transition-all duration-200 group-hover:gap-4"
                  style={{ background: '#E5FF00', color: '#0A0A0A', borderRadius: 2 }}
                >
          ...
          <div className="flex flex-col gap-3">
              ...
              return (
                ...
                >
                  <a
                    ...
                    className="group flex flex-col justify-between h-full min-h-[160px] p-6 relative overflow-hidden block transition-all duration-200"
                    style={{
                  ...
                  >
                    ...
                    <div>
                      ...
                      <h4 className="text-lg font-black text-white mt-2 group-hover:text-[#E5FF00] transition-colors tracking-tight">
                        {drop.title}
                      </h4>
                      <p className="text-white/35 text-xs mt-2 leading-relaxed">{drop.description}</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-white/15 group-hover:text-[#E5FF00] group-hover:translate-x-1 transition-all mt-4" />
                  </a>
                </motion.div>
              );
            })}
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 py-3.5 font-mono text-[9px] tracking-[0.3em] text-white/25 hover:text-[#E5FF00] transition-colors uppercase"
