...
export function useGovernanceReadiness({
  ...
  const checks = useMemo(() => {
    ...
    // Export packet generated
    check('export_packet', 'Export packet generated', !!exportPacketExists, 10);

    // Closeout passed
    check('closeout_passed', 'Event closeout validated', !!closeoutPassed, 15, true);

    return list;
  }, [event, sessions, results, officials, techInspections, auditLogs, exportPacketExists, closeoutPassed, dataHealthScore]);
